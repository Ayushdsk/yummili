...
export const googleAuth = (token) => async (dispatch) => {
  try {
    localStorage.setItem("yummiliciousUser", JSON.stringify({ token }));
...
export const signOut = () => async (dispatch) => {
  try {
    localStorage.removeItem("yummiliciousUser");
    clearUser();
...
//     localStorage.setItem("yummiliciousUser", JSON.stringify({token: User.data.token}));
