  ...
  return (
    <div className="flex w-full items-center justify-between md:hidden">
      ...
      <div className="flex items-center gap-3">
        <button className="bg-yummilicious-400 text-white py-2 px-3 rounded-full">
          Use App
        </button>
        {reduxState?.user?.fullname ? (
          <>
            <div
              ...
              className="border p-2 border-gray-300 text-yummilicious-400 rounded-full w-20 h-20"
            >
              <img
          ...
          <>
            <div
              onClick={() => setIsDropDownOpen((prev) => !prev)}
              className="border p-2 border-gray-300 text-yummilicious-400 rounded-full"
            >
              <FaUserAlt />
            </div>
            {isDropDownOpen && (
        ...
        <div className="hidden w-full gap-4 items-center justify-between lg:flex">
          ...
          <div className=" w-3/4 bg-white shadow-md p-3 flex items-center gap-3 border border-grey-300 rounded-md">
            <div className="flex items-center gap-2 border-r-2 border-grey-300 pr-2">
              <span className="text-yummilicious-300 text-2xl">
                <HiLocationMarker />
              </span>
              <input
                type="text"
          ...
          {reduxState?.user?.fullname ? (
            <div className="relative w-20">
              <div
                onClick={() => setIsDropDownOpen((prev) => !prev)}
                className="border p-2 border-gray-300 text-yummilicious-400 w-full h-20 rounded-full"
              >
                <img
                  src={gravatar.url(reduxState?.user?.email)}
                  alt={reduxState?.user?.email}
